...
from BERT.BERTWithDataPrep import split_user_data
...
def process_dataset(group_keys, grouped_data, negative_data, mode):
    ...
    columns = [f'{i+1}th_product_id' for i in range(5)] + [f'{i+1}th_concurrent_purchase' for i in range(5)] + ['6th_product_id', 'label']
    data_rows = []
...
def create_new_row(last_5_purchases, next_purchase, label=1):
    ...
    for i in range(len(last_5_purchases)):
        new_row[f'{i+1}th_product_id'] = last_5_purchases.iloc[i].StockCode
        is_concurrent = 0
        if i > 0 and last_5_purchases.iloc[i].InvoiceDate == last_5_purchases.iloc[i-1].InvoiceDate or \
           i < len(last_5_purchases) - 1 and last_5_purchases.iloc[i].InvoiceDate == last_5_purchases.iloc[i+1].InvoiceDate:
            is_concurrent = 1
        new_row[f'{i+1}th_concurrent_purchase'] = is_concurrent
    
    # Add the next purchase and label
    new_row['6th_product_id'] = next_purchase.StockCode if 'StockCode' in dir(next_purchase) else 0
    new_row['label'] = label
    
    return new_row

# Main data processing flow
data_dir = "data/BERT_ConcurrentPurchases"  # Original data directory
new_data_dir = "data/DeepFM_SinglePurchases"  # New data directory for processed files
...
train_df.to_csv(os.path.join(new_data_dir, 'trainForDF_WOCP.csv'), index=False)
val_df.to_csv(os.path.join(new_data_dir, 'valForDF_WOCP.csv'), index=False)
test_df.to_csv(os.path.join(new_data_dir, 'testForDF_WOCP.csv'), index=False)
